...
public extension VRM1 {
    ...
    struct FirstPerson: Codable {
        ...
        public init(from decoder: Decoder) throws {
            ...
            meshAnnotations = container.contains(.meshAnnotations)
                ? try container.decode([MeshAnnotation].self, forKey: .meshAnnotations)
                : []
            extensions = container.contains(.extensions)
                ? try container.decode(CodableAny.self, forKey: .extensions)
                : nil
            extras = container.contains(.extras)
                ? try container.decode(CodableAny.self, forKey: .extras)
                : nil
        }
        ...
        public struct Preset: Codable {
            ...
            public init(from decoder: Decoder) throws {
                ...
                let fallback = Expression(
                    morphTargetBinds: nil,
                    materialColorBinds: nil,
                    textureTransformBinds: nil,
                    isBinary: nil,
                    overrideBlink: nil,
                    overrideLookAt: nil,
                    overrideMouth: nil,
                ...
                func decodeExpressionOrFallback(_ key: CodingKeys) throws -> Expression {
                    guard container.contains(key) else {
                        return fallback
                    }
                    return try container.decode(Expression.self, forKey: key)
                }

                happy = try decodeExpressionOrFallback(.happy)
                angry = try decodeExpressionOrFallback(.angry)
                sad = try decodeExpressionOrFallback(.sad)
                relaxed = try decodeExpressionOrFallback(.relaxed)
                surprised = try decodeExpressionOrFallback(.surprised)
                aa = try decodeExpressionOrFallback(.aa)
                ih = try decodeExpressionOrFallback(.ih)
                ou = try decodeExpressionOrFallback(.ou)
                ee = try decodeExpressionOrFallback(.ee)
                oh = try decodeExpressionOrFallback(.oh)
                blink = try decodeExpressionOrFallback(.blink)
                blinkLeft = try decodeExpressionOrFallback(.blinkLeft)
                blinkRight = try decodeExpressionOrFallback(.blinkRight)
                lookUp = try decodeExpressionOrFallback(.lookUp)
                lookDown = try decodeExpressionOrFallback(.lookDown)
                lookLeft = try decodeExpressionOrFallback(.lookLeft)
                lookRight = try decodeExpressionOrFallback(.lookRight)
                neutral = try decodeExpressionOrFallback(.neutral)
            }
        }
